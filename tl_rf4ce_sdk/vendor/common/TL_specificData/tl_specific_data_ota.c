 ...
 *
 * @brief   This is the source file for tl_specific_data_ota.c
 *
 * @author	Zigbee GROUP
 * @date    2021
 *
...
//			req->totalImageSize = rc_otaInfo.totalImageSize;
			...
			mso_vendorSpecficDataSend(ota_pairingRef, (u8 *)msg, sizeof(tl_appFrameHdr_t) + sizeof(ota_preamble_t)-4, 1);
#endif
			ev_buf_free((u8 *)msg);
		}else{
			while(1);
...
#elif (RF4CE_MSO_ENABLE)
		mso_vendorSpecficDataSend(ota_pairingRef, (u8 *)msg, sizeof(tl_appFrameHdr_t) + sizeof(ota_preamble_t), 1);
#endif
...
#elif (RF4CE_MSO_ENABLE)
			mso_vendorSpecficDataSend(ota_pairingRef, (u8 *)msg, sizeof(tl_appFrameHdr_t) + sizeof(ota_dataFrame_t)+read_len, 1);
#endif
			ev_buf_free((u8 *)msg);
		}else{
			while(1);
...
#elif (RF4CE_MSO_ENABLE)
			mso_vendorSpecficDataSend(ota_pairingRef, (u8 *)msg, sizeof(tl_appFrameHdr_t), 1);
#endif
			ev_buf_free((u8 *)msg);
		}else{
			while(1);
...
#elif (RF4CE_MSO_ENABLE)
			mso_vendorSpecficDataSend(ota_pairingRef, (u8 *)msg, sizeof(ota_preamble_t)+sizeof(tl_appFrameHdr_t)-4, 1);
#endif
			ev_buf_free((u8 *)msg);
		}else{
			while(1);
